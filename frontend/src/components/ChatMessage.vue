<template>
  <div class="chat-message" :class="[message.role, { error: message.isError }]">
    ...
    <div class="msg-body">
      ...
      <!-- 数据图表 -->
      <DataChart v-if="message.chartData" :chartData="message.chartData" />

      <!-- 问数流程追踪 -->
      <WorkflowTrace v-if="message.role === 'assistant'" :message="message" />
  ...
  try {
    ...
    if (!text) {
      return props.message.isStreaming
        ? '<span class="stream-placeholder">正在生成…</span>'
        : "";
    }
