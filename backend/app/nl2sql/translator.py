...
NL2SQL_SYSTEM_PROMPT = """你是 SQLite 查询生成器。根据用户问题和表结构生成 SELECT 语句。
...
class NL2SQLTranslator:
    ...
    def translate(
        ...
        # 构建示例：优先使用 RAG 检索到的相关示例，回退到默认示例
        if rag_context:
            rag_examples = self._extract_examples_from_context(rag_context)
            if rag_examples:
        ...
        system_prompt = NL2SQL_SYSTEM_PROMPT.format(schema=schema, examples=examples)
...
class AnswerGenerator:
    ...
    ANSWER_SYSTEM_PROMPT = """你是一个专业的数据分析助手。根据 SQL 查询结果生成清晰、专业的回答。
...
class RecommendedQuestionsGenerator:
    ...
    RECOMMEND_SYSTEM_PROMPT = """你是一个电商数据问答系统的推荐引擎。根据用户刚才的提问和系统回答，生成3个用户可能感兴趣的后续问题。

## 可用数据
- 电商订单、客户、商品、月度目标、退款数据(2023-2024)
- 可分析维度: 品类、省份、会员、支付、折扣、退款、达成率
- 可分析指标: 销售额、客单价、增长率、同比环比、排名

## 规则
1. 问题要基于实际表结构，确保可回答
2. 问题要多样化，从不同角度深挖数据
3. 优先推荐能生成图表的问题（趋势、排名、对比、占比）
4. 问题要简洁明了，20字以内
5. 用 JSON 数组格式输出，如: ["问题1", "问题2", "问题3"]
