...
class TestNL2SQLTranslator:
    ...
    def test_translate_simple_question(self, translator):
        """简单聚合查询"""
        schema = "表 [orders]: 订单ID, 日期, 订单金额_元"
        sql = translator.translate("订单总数是多少", schema)
        assert sql
        assert "SELECT" in sql.upper()
        assert "COUNT" in sql.upper()

    def test_translate_join_question(self, translator):
        """JOIN 查询"""
        schema = "表 [orders]: 订单ID, 客户ID\n表 [customers]: 客户ID, 姓名"
        sql = translator.translate("每个客户的订单数", schema)
        assert sql
        assert "SELECT" in sql.upper()

    def test_translate_returns_only_sql(self, translator):
        """返回纯净 SQL"""
        schema = "表 [orders]: 订单ID, 日期, 订单金额_元"
        sql = translator.translate("订单总数", schema)
        assert sql
        # 不应该包含 markdown 代码块
...
class TestAnswerGenerator:
    ...
    def test_generate_with_data(self, generator):
        """有数据时生成回答"""
    ...
    def test_generate_empty_result(self, generator):
        """空结果时生成回答"""
        result = generator.generate(
            question="不存在的数据",
